...
include_once APPPATH."libraries/UAParserInterface.php";
include_once APPPATH."libraries/Result/ResultFactory.php";
	...
	*/
	public function __construct($regexesPath = null)
	{
		...
		if (null === $regexesPath) 
		{
			...
			$regexesPath = APPPATH."config/regexes.yml";
		}

		// $this->regexes = Yaml::parse(file_get_contents($regexesPath));
		$this->regexes = $CI->yaml->load(file_get_contents($regexesPath));
	}
	...
	protected function parseBrowser($userAgent)
	{
		...
		foreach ($this->regexes['browser_parsers'] as $expression) 
		{
			if (preg_match('/'.str_replace('/','\/',str_replace('\/','/', $expression['regex'])).'/i', $userAgent, $matches)) 
			{
				...
				$result['family'] = isset($expression['family_replacement']) 
						? str_replace('$1', $matches[1], $expression['family_replacement']) 
						: $matches[1];

				$result['major']  = isset($expression['major_replacement']) 
						? $expression['major_replacement'] 
						: $matches[2];

				$result['minor']  = isset($expression['minor_replacement']) 
						? $expression['minor_replacement'] 
						: $matches[3];

				$result['patch']  = isset($expression['patch_replacement']) 
						? $expression['patch_replacement'] 
						: $matches[4];
	...
	protected function parseRenderingEngine($userAgent)
	{
		...
		foreach ($this->regexes['rendering_engine_parsers'] as $expression) 
		{
			if (preg_match('/'.str_replace('/','\/',str_replace('\/','/', $expression['regex'])).'/i', $userAgent, $matches)) 
			{
				...
				$result['family'] = isset($expression['family_replacement']) 
						? str_replace('$2', $matches[2], $expression['family_replacement']) 
						: $matches[1];

				$result['version'] = isset($expression['version_replacement']) 
						? str_replace('$1', $matches[1], $expression['version_replacement']) 
						: $matches[2];
